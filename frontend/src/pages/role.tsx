...
import { Content, FullContent, Infobar } from '../layouts'
                ...
                <div className="flex flex-col h-full w-full justify-start items-center border-r-2 border-gray-600">
                    <div className="flex bg-[#1d3557] flex-none w-full h-20 justify-end items-center px-6">
                        <div className="flex gap-4">
                            ...
                            <button className="bg-[#37AFE2] rounded-md text-white py-2 px-4 flex gap-3 items-center justify-center" onClick={handleAddRole}><BsFillPersonPlusFill size={26} /> Add Role</button>
                        </div>
                    </div>
                    {users.map(user => (
                        <div className="flex flex-col justify-center w-full p-2 overflow-y-auto">
                            <UserCard key={user.id} name={user.fullName || "Eyoel Meles"} email={user.email || "tojoelmeles@gmail.com"} image={user.image || "https://avatars.githubusercontent.com/u/106088179?s=400&u=08ae898ec6940ef2d4c7a30911153081b34340a3&v=4"} handleUser={() => handleAssignRole(user.id) } />
                        </div>
                    ))}
                </div>
            </Content>
            
            {addRole ? (<AddRoleForm />) :(users.length !== 0 && (
                <Infobar title="Edit">
                    <div className="flex justify-evenly p-4">
                        {assignRole && <AssignRoleForm user={user} /> }
                    </div>
                </Infobar>)
            )}
        </div>
